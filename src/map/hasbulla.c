...
void display_hasbulla(mobe_t *mob, main_game_t *game)
{
    ...
    if (mob->attack == true && mob->attack_clock->seconds >= 0.08) {
        mob->object->position.x += (game->player->object->position.x -
        mob->object->position.x) / (sqrt(pow(game->player->object->position.x -
        mob->object->position.x, 2) + pow(game->player->object->position.y -
        mob->object->position.y, 2)));
        mob->object->position.y += (game->player->object->position.y -
        mob->object->position.y) / (sqrt(pow(game->player->object->position.x -
        mob->object->position.x, 2) + pow(game->player->object->position.y -
        mob->object->position.y, 2)));
